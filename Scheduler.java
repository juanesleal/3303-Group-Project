...
public class Scheduler {
    ...
    public static void main(String[] args) {
        ...
        boolean elevatorHasBeenSelected = false;
        Scheduler s;
        if (args.length != 0) {
        ...
        while(true) {
            ...
            switch (s.currentState) {
                ...
                case ELEVWAIT:
                    ...
                    if (m.getData()[0].equals("Availible")) {
                        if (s.next == (s.timeTillRequest.length - 1)){
                            ...
                            s.next = 1;
                            //great, send ok
                            //make sure the scheduler knows an elevator has been selected
                            elevatorHasBeenSelected = true;
                    ...
                    if (s.timeTillRequest[0].length() > 1) {
                        ...
                        if (m.getData()[0].equals("OK")) {
                        	...
                        	while(!m.getData()[0].equals("OK")) {
                        		...
                        		if(elevatorHasBeenSelected) {
                        					String elevatorSelected = m.getToFrom();
                        			}
                        		}
                ...
                case SCHEDULING:
                   ...
                   if (m.getData()[0].equals("OK")) {
                         m = s.eCommunicator.receive();
                         if (m.getData()[0].equals("Available")) {
                             //FIXME
                             m = s.eCommunicator.rpc_send(new Message(new String[] {"timeFor", "5"}, time.millis(), m.getToFrom()));
                         }
                     }else {
    ...
    enum State {
        INIT, EVENTWAIT, ELEVWAIT, SCHEDULING, GETDATA;
    }
    private State next() {
